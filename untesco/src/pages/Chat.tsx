...
const ANALOGY = 'Представь квадратное уравнение как траекторию брошенного мяча. Корни — точки, где мяч касается земли. Бросили слишком высоко — касаний нет (нет корней). Едва касается — один корень. Пролетает насквозь — два корня.'
...
export default function Chat() {
  ...
  const send = () => {
    ...
    setMessages(p => [...p, { role:'user', text, time:ts() }])
    setInput('')
  ...
  return (
    <div className="chat-wrap">
      ...
      <button className="chat-analogy-btn" onClick={() => { if (!used) { setUsed(true); setMessages(p => [...p, { role:'ai', isAnalogy:true, analogyText:ANALOGY, time:ts() }]) } }} disabled={used}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        {used ? 'Аналогия отправлена' : 'Попросить аналогию'}
      </button>
