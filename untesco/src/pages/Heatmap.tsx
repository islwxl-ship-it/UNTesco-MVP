...
const OVERALL = 67
const R = 37, CIRCUM = 2 * Math.PI * R
const cls = (p: number) => p >= 75 ? { bar:'pbar-green', clr:'clr-green' } : p >= 40 ? { bar:'pbar-amber', clr:'clr-amber' } : { bar:'pbar-red', clr:'clr-red' }
  ...
  return (
    <div className="page">
      ...
      <div className="ring-card card">
        <div className="ring-wrap">
          <svg className="ring-svg" width="90" height="90" viewBox="0 0 90 90">
            <defs><linearGradient id="rg" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#6690F9"/><stop offset="100%" stopColor="#1D3DBF"/></linearGradient></defs>
            <circle className="ring-track" cx="45" cy="45" r={R}/>
            <circle ref={ringRef} className="ring-fill" cx="45" cy="45" r={R} strokeDasharray={CIRCUM} strokeDashoffset={CIRCUM}/>
          </svg>
          <div className="ring-center"><div className="ring-pct">{OVERALL}%</div><div className="ring-lbl">Прогресс</div></div>
        </div>
      ...
      <div className="achilles-card">
        ...
        <div><div className="achilles-ttl">Ахиллесова пята</div><div className="achilles-name">Логарифмы</div><div className="achilles-desc">23% усвоения · Требует внимания</div></div>
        <Link href="/chat"><button className="achilles-btn">Учить</button></Link>
      </div>
      <div className="topics-list">
        {TOPICS.map((t, i) => {
          const { bar, clr } = cls(t.pct)
          return (
            <div key={t.name} className="heat-row">
              <div className="heat-row-top"><span className="heat-row-name">{t.name}</span><span className={`heat-row-pct ${clr}`}>{t.pct}%</span></div>
              <div className="pbar-wrap">
                <div className={`pbar-fill ${bar}`} style={{ width:0, transition:`width 0.6s ease ${i*55}ms` }}
